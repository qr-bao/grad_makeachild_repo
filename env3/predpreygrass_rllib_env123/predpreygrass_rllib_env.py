...
class PredPreyGrass(MultiAgentEnv):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        for i, grass in enumerate(self.grass_agents):
            ...
            self.grass_positions[grass] = position
            self.grass_energies[grass] = self._sample_grass_energy()
            
            # 初始化草的属性
            self.grass_age[grass] = 0
            self.grass_generation[grass] = 0
            if self.enable_continuous_space and self.space is not None:
                ...
                shape.sensor = True
                shape.grass_id = grass
                
                self.grass_bodies[grass] = body
                self.grass_shapes[grass] = shape
                
                self.space.add(body, shape)
                
                if self.verbose_spawning:
                    # print(f"[INIT] Grass {grass} created at {position}, sensor={shape.sensor}")
                    pass
            else:
                grid_pos = tuple(int(coord) for coord in position)
                self.grid_world_state[3, *grid_pos] = self.grass_energies[grass]
