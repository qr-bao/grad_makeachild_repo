...
from predpreygrass.rllib.env3.predpreygrass_rllib_env123.observation_visualizer import ObservationSpaceVisualizer
...
class PredPreyVisualizer:
    ...
    def __init__(
        ...
        self.record_frame_interval = max(1, record_frame_interval)
        self.record_dir = Path(record_dir).expanduser() if record_dir else None
        self._frame_output_dir: Optional[Path] = None
        ...
        if self.record_dir:
            ...
            self._frame_output_dir = self.record_dir / "frames"
            self._frame_output_dir.mkdir(parents=True, exist_ok=True)
            print(f"🎬 Recording frames to: {self._frame_output_dir}")
    ...
    def close(self):
        ...
        if self._frame_output_dir:
            print(f"🎞️ Frames stored in: {self._frame_output_dir}")
    
    def _maybe_record_frame(self):
        """按需将当前帧保存成 PNG"""
        if not self._frame_output_dir:
            return
        if self.total_frames_rendered % self.record_frame_interval != 0:
            return
        frame_path = self._frame_output_dir / f"frame_{self.recorded_frames:06d}.png"
        pygame.image.save(self.screen, frame_path.as_posix())
