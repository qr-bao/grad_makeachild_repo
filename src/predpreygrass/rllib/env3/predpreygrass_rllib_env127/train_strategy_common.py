...
from predpreygrass.rllib.env3.predpreygrass_rllib_env127 import (
    train_simple as base_train,
)
from predpreygrass.rllib.env3.predpreygrass_rllib_env127.metrics_callbacks import (
    EpisodeMetricsCallbacks,
)
from predpreygrass.rllib.env3.predpreygrass_rllib_env127.random_baseline_trainable import (
    RandomBaselineTrainable,
)
from predpreygrass.rllib.env3.predpreygrass_rllib_env127.torch_random_rl_module import (
    TorchRandomRLModule,
)
...
def parse_args(default_predator_strategy: StrategyLiteral, default_prey_strategy: StrategyLiteral) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of evaluation workers (parallel env runners).",
    )
...
def run_training(
    ...
    if random_only_mode:
        ...
        if args.eval_interval > 0 and args.eval_episodes > 0:
            ...
            logger.info(
                "Evaluation enabled (interval=%s, episodes=%s, workers=%s)",
                args.eval_interval,
                args.eval_episodes,
