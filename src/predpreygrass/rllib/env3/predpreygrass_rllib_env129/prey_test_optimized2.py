...
BALANCED_CONFIG_PATH = (
    Path(__file__).resolve().parent / "config" / "balanced_config_5000.json"
)

if BALANCED_CONFIG_PATH.exists():
    try:
        with BALANCED_CONFIG_PATH.open("r", encoding="utf-8") as fp:
            prey_test_config = json.load(fp)
        print(f"[INFO] Loaded balanced config from {BALANCED_CONFIG_PATH}")
    except Exception as exc:
        print(
            f"[WARNING] Failed to load {BALANCED_CONFIG_PATH}, falling back to config_env_base: {exc}"
        )
        prey_test_config = config_env_base
else:
    print(
        f"[WARNING] {BALANCED_CONFIG_PATH} not found; using config_env_base defaults."
    )
    prey_test_config = config_env_base


class PredatorChasePolicy:
    """
    简单的追猎策略：
        1. 寻找最近的活跃 prey
        2. 预测其短期运动趋势
        3. 以受限推力朝预测点加速
        4. 若附近无 prey，则使用随机游走兜圈
    """
    ...
    def __init__(
        ...
        lead_time: float = 0.4,
        max_gain: float = 0.5,
        close_gain: float = 0.18,
        brake_distance: float = 35.0,
    ):
        self.env = env
    ...
    def _get_predator_pos(self, agent_id: str) -> Optional[np.ndarray]:
        pos = self.env.agent_positions.get(agent_id)
        if pos is None:
            return None
        return np.array(pos, dtype=np.float32)

    def _iter_prey_targets(self):
        for prey_id in self.env.agents:
            if "prey" not in prey_id:
                continue
            pos = self.env.agent_positions.get(prey_id)
            if pos is None:
                continue
            yield prey_id, np.array(pos, dtype=np.float32)

    def _predict_prey_future(self, prey_id: str, prey_pos: np.ndarray, distance: float) -> np.ndarray:
        body = self.env.agent_bodies.get(prey_id)
        if body is None:
            return prey_pos

        velocity = np.array([body.velocity.x, body.velocity.y], dtype=np.float32)
        predator_speed = getattr(self.env, "direct_velocity_speed_predator", 200.0)
        time_scale = min(self.lead_time, distance / max(predator_speed, 1e-3))
        return prey_pos + velocity * time_scale

    def _compute_direction(self, predator_pos: np.ndarray, target_pos: np.ndarray) -> np.ndarray:
        vector = target_pos - predator_pos
        distance = float(np.linalg.norm(vector) + 1e-8)
        direction = vector / distance
        gain = self.max_gain
        if distance < self.brake_distance:
            gain = min(self.close_gain, gain)
        return np.clip(direction * gain, -0.5, 0.5).astype(np.float32)

    def get_action(self, agent_id: str) -> np.ndarray:
        predator_pos = self._get_predator_pos(agent_id)
        if predator_pos is None:
            return np.zeros(2, dtype=np.float32)

        best_target = None
        best_distance = float("inf")
        for prey_id, prey_pos in self._iter_prey_targets():
            diff = prey_pos - predator_pos
            distance = float(np.linalg.norm(diff))
            if distance < best_distance:
                best_target = (prey_id, prey_pos, distance)
                best_distance = distance

        if best_target is None:
            # 没有猎物，采用随机巡逻
            return self.wanderer.get_action(agent_id)

        prey_id, prey_pos, distance = best_target
        predicted = self._predict_prey_future(prey_id, prey_pos, distance)
        return self._compute_direction(predator_pos, predicted)
