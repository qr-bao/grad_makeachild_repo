...
from typing import Any, Dict as TypingDict, Iterable, List as TypingList, Set
...
class EpisodeMetricsTracker:
    """Collects per-episode statistics for downstream evaluation."""
    ...
    def reset(
        self,
        prey_ids: TypingList[AgentID],
        pred_ids: TypingList[AgentID],
        prey_energy_total: float,
        pred_energy_total: float,
    ) -> None:
        self.step_count = 0
        self.prey_initial_count = len(prey_ids)
        self.pred_initial_count = len(pred_ids)

        self.prey_sum_n = 0.0
        self.prey_sum_n2 = 0.0
        self.pred_sum_n = 0.0
        self.pred_sum_n2 = 0.0

        self.prey_sum_e_div_n = 0.0
        self.pred_sum_e_div_n = 0.0

        self.prey_energy_gain_total = 0.0
        self.prey_energy_cost_total = 0.0
        self.pred_energy_gain_total = 0.0
        self.pred_energy_cost_total = 0.0

        self.prey_eat_grass_count = 0
        self.pred_catch_prey_count = 0
        self.grass_consumed_count = 0
        self.grass_regrown_count = 0

        self.prey_lifespan_sum = 0.0
        self.prey_lifespan_count = 0
        self.pred_lifespan_sum = 0.0
        self.pred_lifespan_count = 0

        self.prey_extinct_step = None
        self.pred_extinct_step = None

        self.prey_birth_steps: TypingDict[AgentID, int] = {agent: 0 for agent in prey_ids}
        self.pred_birth_steps: TypingDict[AgentID, int] = {agent: 0 for agent in pred_ids}
        self.prey_alive_ids: Set[AgentID] = set(prey_ids)
        self.pred_alive_ids: Set[AgentID] = set(pred_ids)

        self.prev_prey_energy_total = prey_energy_total
        self.prev_pred_energy_total = pred_energy_total

    def register_birth(self, agent_id: AgentID, birth_step: int) -> None:
        if "predator" in agent_id:
            self.pred_birth_steps[agent_id] = birth_step
            self.pred_alive_ids.add(agent_id)
        elif "prey" in agent_id:
            self.prey_birth_steps[agent_id] = birth_step
            self.prey_alive_ids.add(agent_id)

    def record_prey_eat_grass(self) -> None:
        self.prey_eat_grass_count += 1
        self.grass_consumed_count += 1

    def record_predator_catch_prey(self) -> None:
        self.pred_catch_prey_count += 1

    def record_grass_regrown(self) -> None:
        self.grass_regrown_count += 1

    def _record_energy_totals(self, prey_total: float, pred_total: float) -> None:
        if self.prev_prey_energy_total is not None:
            delta_prey = prey_total - self.prev_prey_energy_total
            if delta_prey >= 0:
                self.prey_energy_gain_total += delta_prey
            else:
                self.prey_energy_cost_total += -delta_prey
        if self.prev_pred_energy_total is not None:
            delta_pred = pred_total - self.prev_pred_energy_total
            if delta_pred >= 0:
                self.pred_energy_gain_total += delta_pred
            else:
                self.pred_energy_cost_total += -delta_pred
        self.prev_prey_energy_total = prey_total
        self.prev_pred_energy_total = pred_total

    def record_step(
        self,
        step_index: int,
        prey_ids: Iterable[AgentID],
        pred_ids: Iterable[AgentID],
        prey_energy_total: float,
        pred_energy_total: float,
    ) -> None:
        prey_ids = set(prey_ids)
        pred_ids = set(pred_ids)
        prey_count = len(prey_ids)
        pred_count = len(pred_ids)

        self.step_count = step_index
        self.prey_sum_n += prey_count
        self.prey_sum_n2 += prey_count ** 2
        self.pred_sum_n += pred_count
        self.pred_sum_n2 += pred_count ** 2

        if prey_count > 0:
            self.prey_sum_e_div_n += prey_energy_total / prey_count
        if pred_count > 0:
            self.pred_sum_e_div_n += pred_energy_total / pred_count

        if prey_count == 0 and self.prey_extinct_step is None:
            self.prey_extinct_step = step_index
        if pred_count == 0 and self.pred_extinct_step is None:
            self.pred_extinct_step = step_index

        self._record_energy_totals(prey_energy_total, pred_energy_total)
        self._update_lifespans(prey_ids, pred_ids, step_index)

    def _update_lifespans(
        self,
        current_prey_ids: Set[AgentID],
        current_pred_ids: Set[AgentID],
        step_index: int,
    ) -> None:
        removed_prey = self.prey_alive_ids - current_prey_ids
        for agent_id in removed_prey:
            birth = self.prey_birth_steps.pop(agent_id, step_index)
            self.prey_lifespan_sum += max(0, step_index - birth)
            self.prey_lifespan_count += 1
        self.prey_alive_ids = set(current_prey_ids)

        removed_pred = self.pred_alive_ids - current_pred_ids
        for agent_id in removed_pred:
            birth = self.pred_birth_steps.pop(agent_id, step_index)
            self.pred_lifespan_sum += max(0, step_index - birth)
            self.pred_lifespan_count += 1
        self.pred_alive_ids = set(current_pred_ids)

    def finalize_survivors(self, final_step: int) -> None:
        for agent_id in list(self.prey_alive_ids):
            birth = self.prey_birth_steps.pop(agent_id, final_step)
            self.prey_lifespan_sum += max(0, final_step - birth)
            self.prey_lifespan_count += 1
        self.prey_alive_ids.clear()

        for agent_id in list(self.pred_alive_ids):
            birth = self.pred_birth_steps.pop(agent_id, final_step)
            self.pred_lifespan_sum += max(0, final_step - birth)
            self.pred_lifespan_count += 1
        self.pred_alive_ids.clear()

    def build_metrics(self) -> Dict[str, float]:
        return {
            "episode_steps": self.step_count,
            "prey_initial_count": self.prey_initial_count,
            "pred_initial_count": self.pred_initial_count,
            "prey_sum_N": self.prey_sum_n,
            "prey_sum_N2": self.prey_sum_n2,
            "pred_sum_N": self.pred_sum_n,
            "pred_sum_N2": self.pred_sum_n2,
            "prey_sum_E_div_N": self.prey_sum_e_div_n,
            "pred_sum_E_div_N": self.pred_sum_e_div_n,
            "prey_energy_gain_total": self.prey_energy_gain_total,
            "prey_energy_cost_total": self.prey_energy_cost_total,
            "pred_energy_gain_total": self.pred_energy_gain_total,
            "pred_energy_cost_total": self.pred_energy_cost_total,
            "prey_eat_grass_count": self.prey_eat_grass_count,
            "pred_catch_prey_count": self.pred_catch_prey_count,
            "grass_consumed_count": self.grass_consumed_count,
            "grass_regrown_count": self.grass_regrown_count,
            "prey_lifespan_sum": self.prey_lifespan_sum,
            "prey_lifespan_count": self.prey_lifespan_count,
            "pred_lifespan_sum": self.pred_lifespan_sum,
            "pred_lifespan_count": self.pred_lifespan_count,
            "prey_extinct_step": self.prey_extinct_step,
            "pred_extinct_step": self.pred_extinct_step,
        }
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _population_snapshot(self) -> Tuple[List[AgentID], List[AgentID], float, float]:
        """Return lists of prey/predator IDs and their total energies."""
        prey_ids = list(self.prey_positions.keys())
        pred_ids = list(self.predator_positions.keys())
        prey_energy_total = float(sum(self.agent_energies.get(agent, 0.0) for agent in prey_ids))
        pred_energy_total = float(sum(self.agent_energies.get(agent, 0.0) for agent in pred_ids))
        return prey_ids, pred_ids, prey_energy_total, pred_energy_total

    def _record_metrics_after_step(self) -> None:
        prey_ids, pred_ids, prey_energy_total, pred_energy_total = self._population_snapshot()
        step_index = self.current_step + 1
        self.metrics.record_step(step_index, prey_ids, pred_ids, prey_energy_total, pred_energy_total)
    ...
    def _finalize_episode_metrics(self, infos: Dict[AgentID, Dict[str, Any]]) -> None:
        ...
        if metrics_payload:
            ...
            for agent in self.agents:
                infos.setdefault(agent, {}).setdefault("episode_metrics", metrics_payload)
        self._episode_metrics_finalized = True

    def __init__(self, config=None):
        super().__init__()
        base_config = config or default_env_config
        config = dict(base_config)  # ensure we don't mutate the module-level defaults
        ...
        if self.enable_continuous_space:
            ...
            base_sensor_range = config.get("sensor_range", 150.0)
            self.sensor_range_predator = config.get(
                "sensor_range_predator", base_sensor_range
            )
            self.sensor_range_prey = config.get(
                "sensor_range_prey", base_sensor_range
            )
            # backwards-compatible aggregate value (used by tools/visualizers)
            self.sensor_range = max(
                self.sensor_range_predator, self.sensor_range_prey
            )
            self._sensor_shape_filter = pymunk.ShapeFilter()
            self._sensor_origin_offset = float(self.agent_radius) + 1.0
        else:
            # 保持属性总是存在，便于后续缓存逻辑
            self.n_sensors = 0
            self.sensor_range_predator = 0.0
            self.sensor_range_prey = 0.0
            self.sensor_range = 0.0
            self._sensor_shape_filter = None
            self._sensor_origin_offset = 0.0
        self._rebuild_sensor_cache()
        ...
        respawn_delay = int(config.get("grass_respawn_delay", 200))
        self.grass_respawn_delay = max(0, respawn_delay)
        self.grass_fixed_respawn_delay = self.grass_respawn_delay
        ...
        self.pending_grass_respawns: List[Dict[str, Any]] = []
        ...
        self.thrust_penalty_scale_predator = float(
            config.get("thrust_penalty_scale_predator", 0.0)
        )
        self.thrust_penalty_scale_prey = float(
            config.get("thrust_penalty_scale_prey", 0.0)
        )
        ...
        self.use_direct_velocity_control = bool(config.get("use_direct_velocity_control", False))
        self.direct_velocity_speed_predator = config.get("direct_velocity_speed_predator", None)
        self.direct_velocity_speed_prey = config.get("direct_velocity_speed_prey", None)
        self.direct_velocity_accel_scale_predator = config.get(
            "direct_velocity_accel_scale_predator", None
        )
        self.direct_velocity_accel_scale_prey = config.get(
            "direct_velocity_accel_scale_prey", None
        )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.enable_continuous_space and self.space is not None:
            ...
            # 当 enable_agent_collision=False 时，直接忽略同类碰撞（可穿透）
            handler_predator_predator.pre_solve = (
                lambda arbiter, space, data: self.enable_agent_collision
            )
    ...
    def step(self, action_dict):
        ...
        if self.fixed_grass_mode:
            ...
            self._process_pending_grass_respawns()
        # === 连续空间：基础代谢消耗和伤害 ===
        if self.enable_continuous_space:
        ...
        if self.enable_continuous_space and self.space is not None:
            ...
            for agent in list(self.agents):
                if agent in self.agent_bodies:
                    ...
                    if self.use_direct_velocity_control:
                        # 直接速度控制只在需要时裁剪速度，不额外施加阻尼
                        limit = (
                            self.direct_velocity_speed_predator
                            if "predator" in agent
                            else self.direct_velocity_speed_prey
                        )
                        if limit is None:
                            limit = self.soft_speed_limit_predator if "predator" in agent else self.soft_speed_limit_prey
                        if limit is not None and speed > limit and speed > 0.0:
            ...
            for collision in self.collisions_this_step:
                ...
                if collision_type == 'predator_catch_prey':
                    ...
                    if predator_agent in self.agents and prey_agent in self.agents:
                        self.metrics.record_predator_catch_prey()
                        if self.verbose_engagement:
                ...
                # === 修改：猎物吃草（适配连续空间）===
                    ...
                    if prey_agent in self.agents and grass_id in self.grass_energies:
                        if self.grass_energies[grass_id] > 0:
                            self.metrics.record_prey_eat_grass()
                            if self.verbose_engagement:
            ...
            if self.agent_energies[agent] <= 0:
                ...
                if caught_prey:
                    ...
                    if self.enable_continuous_space and hasattr(self, 'thrust_costs'):
                        thrust_penalty = (
                            self.thrust_costs.get(agent, 0.0)
                            * self.thrust_penalty_scale_predator
                        )
                ...
                if terminations.get(agent) is None or not terminations[agent]:
                    ...
                    if self.enable_continuous_space:
                        ...
                        for grass, _ in self._iter_grass_neighbors(prey_position, eating_distance):
                            if self.grass_energies.get(grass, 0) > 0:
                                caught_grass = grass
                                break
                    else:
                        caught_grass = next(
        ...
        population_extinct = (
            (self.current_num_prey <= 0)
            or (self.current_num_predators <= 0)
        )
        ...
        done_agents = {agent for agent, flag in terminations.items() if flag and agent != "__all__"}
        truncated_agents = {
            agent for agent, flag in truncations.items() if flag and agent != "__all__"
        }
        active_and_done_agents = set(self.agents) | done_agents | truncated_agents

        observations = {
            agent: observations[agent]
            for agent in active_and_done_agents
            if agent in observations
        }
        rewards = {
            agent: rewards.get(agent, 0.0) for agent in active_and_done_agents
        }

        env_terminated = terminations.get("__all__", False)
        env_truncated = truncations.get("__all__", False)
        terminations = {
            agent: terminations.get(agent, False) for agent in active_and_done_agents
        }
        truncations = {
            agent: truncations.get(agent, False) for agent in active_and_done_agents
        }
        terminations["__all__"] = env_terminated
        truncations["__all__"] = env_truncated
        ...
        self._record_metrics_after_step()
    ...
    def _get_move(self, agent: AgentID, action) -> Tuple[float, float]:
        ...
        if self.enable_continuous_space and agent in self.agent_bodies:
            ...
            # 缩放推力到物理单位
            base_scale = self.thrust_scale_predator if "predator" in agent else self.thrust_scale_prey
            body = self.agent_bodies[agent]

            if self.use_direct_velocity_control:
                # 直接速度控制：动作作为加速度增量，随后裁剪速度
                max_speed = (
                    self.direct_velocity_speed_predator
                    if "predator" in agent
                    else self.direct_velocity_speed_prey
                ...
                if max_speed is None:
                    max_speed = base_scale
                accel_scale = (
                    self.direct_velocity_accel_scale_predator
                    if "predator" in agent
                    else self.direct_velocity_accel_scale_prey
                )
                if accel_scale is None:
                    accel_scale = base_scale
                delta_v = pymunk.Vec2d(fx * accel_scale, fy * accel_scale)
                new_velocity = body.velocity + delta_v
                ...
                # 缩放推力到物理单位
                force_x = fx * base_scale
                force_y = fy * base_scale
    ...
    def _rebuild_sensor_cache(self) -> None:
        """Pre-compute sensor angles/directions for reuse during observations."""
        if not self.enable_continuous_space or self.n_sensors <= 0:
            self._sensor_angles = np.empty(0, dtype=np.float32)
            self._sensor_dirs = np.empty((0, 2), dtype=np.float32)
            return
        ...
        self._sensor_angles = angles
        self._sensor_dirs = dirs
    ...
    def _get_sensor_observation(self, agent):
        ...
        # 计算传感器角度（复用缓存避免重复分配）
        if (
            not hasattr(self, "_sensor_angles")
            or len(self._sensor_angles) != self.n_sensors
        ):
            self._rebuild_sensor_cache()
        sensor_angles = self._sensor_angles
        sensor_dirs = self._sensor_dirs
        self_velocity = self._get_agent_velocity(agent)
        ...
        # 依据类型选择传感器范围
        sensor_range = (
            self.sensor_range_predator
            if agent_type == "predator"
            else self.sensor_range_prey
        )

        # 一次性收集所有射线信息
        ray_results = []
        for angle, direction in zip(sensor_angles, sensor_dirs):
        ...
        predator_layer = self._get_predator_layer(ray_results, agent, agent_type, self_velocity)
        prey_layer = self._get_prey_layer(ray_results, agent, agent_type, self_velocity)
        mate_layer = self._get_mate_layer(ray_results, agent_type, agent_pop_id, self_velocity, agent)
        ...
        np.clip(observation, -1.0, 1.0, out=observation)
        return observation
    ...
    def get_state_snapshot(self):
        return {
            ...
            "pending_grass_respawns": [entry.copy() for entry in self.pending_grass_respawns],
        }
    ...
    def restore_state_snapshot(self, snapshot):
        ...
        self.pending_grass_respawns = [entry.copy() for entry in snapshot.get("pending_grass_respawns", [])]
        # === 恢复草的追踪 ===
    ...
    def _cast_sensor_ray(self, start_pos, angle, max_distance, exclude_agent_id=None, direction=None):
        """发射传感器射线（增强版）"""
        ...
        end_x = start_pos[0] + dir_x * max_distance
        end_y = start_pos[1] + dir_y * max_distance
        end_pos = (end_x, end_y)

        # 轻微偏移起点，避免命中自身
        sensor_filter = self._sensor_shape_filter or pymunk.ShapeFilter()
        offset = min(max_distance * 0.99, max(0.0, self._sensor_origin_offset or 0.0))
        start_offset = (
            start_pos[0] + dir_x * offset,
            start_pos[1] + dir_y * offset,
        )

        result = self.space.segment_query_first(start_offset, end_pos, 0, sensor_filter)
        ...
        segment_length = max_distance - offset
        if segment_length <= 0:
            normalized_distance = 1.0
        else:
            normalized_distance = (offset + result.alpha * segment_length) / max(max_distance, 1e-6)
        normalized_distance = float(np.clip(normalized_distance, 0.0, 1.0))
    ...
    def _create_offspring(self, parent1, parent2):
        ...
        self.metrics.register_birth(offspring, self.current_step)
    ...
    def _register_grass_patch(
        self,
        position: Tuple[float, float],
        *,
        parent_grass_id: str | None = None,
        energy: float | None = None,
    ) -> str:
        """在给定位置创建草实体并更新所有追踪结构。"""
        grass_id = f"grass_{next(self._grass_id_counter)}"
        spawn_energy = self._sample_grass_energy() if energy is None else float(energy)
        self.grass_positions[grass_id] = (float(position[0]), float(position[1]))
        self.grass_energies[grass_id] = spawn_energy
        self.grass_age[grass_id] = 0
        if parent_grass_id is not None and parent_grass_id in self.grass_generation:
            self.grass_generation[grass_id] = self.grass_generation[parent_grass_id] + 1
        else:
            self.grass_generation[grass_id] = 0
        if grass_id not in self.grass_agents:
            self.grass_agents.append(grass_id)
        self.current_num_grass += 1
        self.metrics.record_grass_regrown()
        self._mark_grass_index_dirty()
        ...
        if self.enable_continuous_space and self.space is not None:
            ...
            grid_pos = tuple(int(coord) for coord in self.grass_positions[grass_id])
            self.grid_world_state[3, *grid_pos] = spawn_energy

        return grass_id

    def _pick_grass_spawn_position(
        self, reference_position: Tuple[float, float] | None
        ...
        if self.enable_continuous_space:
            ...
            while attempts < self.grass_spawn_max_attempts:
                if reference_position is None:
                    x = self.rng.uniform(grass_radius, self.world_width - grass_radius)
                    y = self.rng.uniform(grass_radius, self.world_height - grass_radius)
                else:
                    angle = self.rng.uniform(0, 2 * np.pi)
                    distance = self.rng.uniform(min_spacing, max_distance)
                    x = reference_position[0] + math.cos(angle) * distance
                    y = reference_position[1] + math.sin(angle) * distance

                if not (grass_radius < x < self.world_width - grass_radius and grass_radius < y < self.world_height - grass_radius):
                    attempts += 1
                    continue

                too_close = False
                for existing_pos in self.grass_positions.values():
                    dist = math.hypot(x - existing_pos[0], y - existing_pos[1])
                    if dist < min_spacing:
                        too_close = True
                        break
                if not too_close:
            ...
            if self.verbose_spawning:
                origin = "random" if reference_position is None else "anchor"
                print(f"[GRASS] Failed to place spawn near {origin} after {attempts} attempts")
            return None
        ...
        return self._find_available_spawn_position(anchor, occupied_positions)

    def _schedule_grass_respawn(self, position: Tuple[float, float] | None) -> None:
        """在非固定模式下为被吃掉的草安排再生。"""
        if position is None or self.grass_respawn_delay <= 0:
            return
        self.pending_grass_respawns.append(
            {
                "position": (float(position[0]), float(position[1])),
                "timer": self.grass_respawn_delay,
            }
        )

    def _process_pending_grass_respawns(self) -> None:
        if not self.pending_grass_respawns:
            return

        remaining: TypingList[Dict[str, Any]] = []
        for entry in self.pending_grass_respawns:
            entry["timer"] -= 1
            if entry["timer"] <= 0:
                spawn_pos = self._pick_grass_spawn_position(entry.get("position"))
                if spawn_pos is None:
                    # 重试
                    entry["timer"] = 1
                    remaining.append(entry)
                    continue
                self._register_grass_patch(spawn_pos)
            else:
                remaining.append(entry)
        self.pending_grass_respawns = remaining

    def _create_grass_offspring(self, parent_grass_id):
        """创建草的后代（尝试多次找位置）"""
        parent_pos = self.grass_positions.get(parent_grass_id)
        if parent_pos is None:
            return None

        offspring_pos = self._pick_grass_spawn_position(parent_pos)
        if offspring_pos is None:
            return None

        return self._register_grass_patch(offspring_pos, parent_grass_id=parent_grass_id)
    ...
    def _remove_grass(self, grass_id: str) -> None:
        ...
        if self.current_num_grass > 0:
            self.current_num_grass -= 1
        self._mark_grass_index_dirty()
        self._schedule_grass_respawn(position)
    @staticmethod
    ...
    def _get_environment_layer(self, ray_results):
        ...
        layer = np.concatenate([obstacle_distances, grass_distances, grass_energies])
        np.clip(layer, -1.0, 1.0, out=layer)
        return layer


    def _get_predator_layer(self, ray_results, self_id, self_type, self_velocity):
        ...
        for i, result in enumerate(ray_results):
            ...
            if velocity != (0, 0):
                rel_vx = velocity[0] - self_velocity[0]
                rel_vy = velocity[1] - self_velocity[1]
                
                ray_dir = result.get('ray_dir')
                if ray_dir:
        ...
        layer = np.concatenate([distances, velocities, energies])
        np.clip(layer, -1.0, 1.0, out=layer)
        return layer


    def _get_prey_layer(self, ray_results, self_id, self_type, self_velocity):
        """Prey层：距离、速度、能量（90维）"""
        distances = np.ones(self.n_sensors, dtype=np.float32)
        velocities = np.zeros(self.n_sensors, dtype=np.float32)
        energies = np.zeros(self.n_sensors, dtype=np.float32)
    ...
    def _get_mate_layer(self, ray_results, self_type, self_pop_id, self_velocity, self_id=None):
        """同类同群层：距离、速度、可交配性（90维）"""
        distances = np.ones(self.n_sensors, dtype=np.float32)
        velocities = np.zeros(self.n_sensors, dtype=np.float32)
        fertility = np.zeros(self.n_sensors, dtype=np.float32)
        if self_velocity is None:
        ...
        layer = np.concatenate([distances, velocities, fertility])
        np.clip(layer, -1.0, 1.0, out=layer)
        return layer
...
# predpreygrass_rllib_env.py - 添加辅助函数（在类中添加，约1900行附近）
    ...
    def _iter_grass_neighbors(self, position: Tuple[float, float], radius: float):
        """Yield grass IDs within the provided radius around position."""
        if not self.grass_positions:
            return

        self._ensure_grass_spatial_index()
        cell_size = max(1.0, getattr(self, "_grass_cell_size", 1.0))
        radius = max(0.0, float(radius))
        if radius == 0.0:
            return

        cx = int(float(position[0]) // cell_size)
        cy = int(float(position[1]) // cell_size)
        cell_range = int(math.ceil(radius / cell_size))
            ...
            for dy in range(-cell_range, cell_range + 1):
                ...
                for grass_id in self._grass_spatial_index.get(cell_key, ()):
                    ...
                    if grass_pos is None:
                        continue
                    diff_x = float(position[0]) - float(grass_pos[0])
                    diff_y = float(position[1]) - float(grass_pos[1])
                    if diff_x * diff_x + diff_y * diff_y <= radius_sq:
                        yield grass_id, grass_pos
